            ...
            for (var i = 0; i < data.GetArrayLength(); i++)
            {
	           var brandId = data[i].GetProperty("id").GetInt32();
                ...
                for (var j = 0; j < data[i].GetProperty("sizes").GetArrayLength(); j++)
                {
	                var size = new Size()
	                {
		                name = data[i].GetProperty("sizes")[j].GetString()
	                };
                ...
                for (var j = 0; j < data[i].GetProperty("colors").GetArrayLength(); j++)
                {
	                var color = new Color()
	                {
		                name = data[i].GetProperty("colors")[j].GetString()
	                };
                ...
                var images_urls = new List<ImageUrl>();
                for (int j = 0; j < data[i].GetProperty("images_urls").GetArrayLength(); j++)
                {
	                var image_url = new ImageUrl()
	                {
		                name = data[i].GetProperty("images_urls")[j].GetString()
	                };
